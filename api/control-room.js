...
// Popular search terms that return diverse Base pairs
const SEARCH_QUERIES = [
  'AERO','BRETT','DEGEN','VIRTUAL','TOSHI','HIGHER','MOCHI','MOG',
  'BASED','NORMIE','WELL','CBBTC','TURBO','PEPE','WOJAK','ODOS',
  'PRIME','LUNA','TRUMP','MFER','ZORA','ENJOY','IMAGINE','LIFE',
];
...
async function fetchAllPairs() {
  // 1. Token profiles (new/featured tokens)
  // 2. Token boosts (popular/promoted tokens)
  // 3. 24 targeted search queries in parallel
  const tasks = [
  ...
  // Collect token addresses from profiles + boosts, then batch-fetch pairs
  const extraAddrs = new Set();
  if (Array.isArray(profilesRes)) {
    for (const t of profilesRes) {
      if (t.chainId === 'base' && t.tokenAddress) extraAddrs.add(t.tokenAddress);
    }
  }
  ...
  const addrList = [...extraAddrs].slice(0, 90);
  const chunks = [];
  for (let i = 0; i < addrList.length; i += 30) {
    chunks.push(addrList.slice(i, i + 30).join(','));
  }
