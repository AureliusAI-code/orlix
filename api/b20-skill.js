...
// ── Network ───────────────────────────────────────────────────────────────────
const RPC_URL  = { mainnet: 'https://mainnet.base.org', sepolia: 'https://sepolia.base.org', vibenet: 'https://rpc.vibes.base.org', robinhood: 'https://rpc.mainnet.chain.robinhood.com/' };
const CHAIN_ID = { mainnet: 8453, sepolia: 84532, vibenet: 84538453, robinhood: 4663 };
const EXPLORER = { mainnet: 'https://basescan.org', sepolia: 'https://sepolia.basescan.org', vibenet: 'https://explorer.vibes.base.org', robinhood: 'https://robinhoodchain.blockscout.com' };
...
async function handleInfo(net, res) {
  try {
    ...
    return res.end(JSON.stringify({
      ok:       true,
      standard: 'B20',
      network:  net === 'mainnet' ? 'Base' : net === 'vibenet' ? 'Base Vibenet' : net === 'robinhood' ? 'Robinhood Chain' : 'Base Sepolia',
...
async function handleBalance(body, res) {
  ...
  const net = ['mainnet', 'sepolia', 'vibenet', 'robinhood'].includes(network) ? network : 'mainnet';
...
async function handleValidate(body, res) {
  ...
  const net = ['mainnet', 'sepolia', 'vibenet', 'robinhood'].includes(body.network) ? body.network : 'mainnet';
...
function handleManifest(res) {
  return res.end(JSON.stringify({
    ...
    networks:    { mainnet: { chainId: 8453, rpc: 'https://mainnet.base.org' }, sepolia: { chainId: 84532, rpc: 'https://sepolia.base.org' }, vibenet: { chainId: 84538453, rpc: 'https://rpc.vibes.base.org', explorer: 'https://explorer.vibes.base.org' }, robinhood: { chainId: 4663, rpc: 'https://rpc.mainnet.chain.robinhood.com/', explorer: 'https://robinhoodchain.blockscout.com' } },
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const net    = ['mainnet', 'sepolia', 'vibenet', 'robinhood'].includes(body.network ?? req.query?.network)
